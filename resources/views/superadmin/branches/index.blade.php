...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Branches for: {{ $school->name }}</h1>
                <a href="{{ route('superadmin.schools.index') }}" class="text-sm text-blue-600 hover:underline">&larr; Back to
                    All Schools</a>
            </div>
            <a href="{{ route('superadmin.schools.branches.create', $school) }}"
                    ...
                    @forelse ($school->branches as $branch)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('superadmin.schools.branches.edit', [$school, $branch]) }}"
                                    class="text-blue-600 hover:text-blue-900">Edit</a>
                                <form action="{{ route('superadmin.schools.branches.destroy', [$school, $branch]) }}"
                                    method="POST" class="inline ml-4" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No
                                branches found for this school.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

    ...
    <div class="max-w-4xl mx-auto px-6 py-10">
        <div class="bg-white shadow-lg rounded-2xl p-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Edit Exam: {{ $exam->name }}</h1>
            </div>

            <form action="{{ route('school-superadmin.exams.update', $exam) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="grade_system_id" id="grade_system_id" required
                            ...
                            @foreach ($gradeSystems as $system)
                                <option value="{{ $system->id }}"
                                    {{ old('grade_system_id', $exam->grade_system_id) == $system->id ? 'selected' : '' }}>
                                    {{ $system->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $exam->end_date) }}"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-500">
                    </div>
                </div>

...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto" x-data="gradeSystemForm()">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Create New Grade System</h1>

        <form action="{{ route('school-superadmin.grade-systems.store') }}" method="POST">
            @csrf
            <div class="space-y-6">
                <!-- Main Details -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" value="{{ old('description') }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                            placeholder="e.g., For classes 9-12">
                    </div>
                </div>
    ...
    @push('scripts')
        <script>
            function gradeSystemForm(existingGrades = []) {
                let initialGrades = existingGrades.length > 0 ? existingGrades : [{
                    grade_name: '',
                    mark_from: '',
                    mark_to: '',
                    comments: ''

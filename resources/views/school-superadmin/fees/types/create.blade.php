...
@section('content')
    <div class="max-w-4xl mx-auto px-6 py-10">
        <div class="bg-white shadow-lg rounded-2xl p-8">
            ...
            {{-- Form --}}
            <form action="{{ route('school-superadmin.fee-types.store') }}" method="POST" class="space-y-6">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Fee Group --}}
                    <div>
                        <label for="fee_group_id" class="block text-sm font-medium text-gray-700 mb-1">
                            Fee Group *
                        </label>
                        <select name="fee_group_id" id="fee_group_id" required
                            ...
                            @foreach ($feeGroups as $group)
                                <option value="{{ $group->id }}"
                                    {{ old('fee_group_id') == $group->id ? 'selected' : '' }}>
                                    {{ $group->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- Fee Code --}}
                    <div>
                        <label for="fee_code" class="block text-sm font-medium text-gray-700 mb-1">
                            Fee Code
                        </label>
                        <input type="text" name="fee_code" id="fee_code" value="{{ old('fee_code') }}"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-500 @error('fee_code') border-red-500 @enderror"
                            placeholder="e.g., TUI-001, BUS-001">
                        @error('fee_code')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-500 @error('description') border-red-500 @enderror"

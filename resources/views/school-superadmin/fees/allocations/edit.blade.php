...
<div class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto" x-data="feeAllocationForm()">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Fee Allocation</h1>
    ...
    <form action="{{ route('school-superadmin.fee-allocations.update', $feeAllocation) }}" method="POST">
        ...
        <div class="space-y-6">
            <div>
                ...
                <select name="branch_id" id="branch_id" x-model="selectedBranchId" @change="loadClasses()" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ old('branch_id', $feeAllocation->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="fee_type_id" id="fee_type_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    @foreach($feeTypes as $type)
                        <option value="{{ $type->id }}" {{ old('fee_type_id', $feeAllocation->fee_type_id) == $type->id ? 'selected' : '' }}>{{ $type->feeGroup->name }} - {{ $type->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', $feeAllocation->amount) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('amount') border-red-500 @enderror" placeholder="0.00">
                @error('amount') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
            
            <div>
                <label for="due_date" class="block text-sm font-medium text-gray-700">Due Date *</label>
                <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $feeAllocation->due_date) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('due_date') border-red-500 @enderror">
    ...
    function feeAllocationForm() {
        return {
            ...
            loadClasses(isInit = false) {
                ...
                // Ensure the pre-selected class is still set

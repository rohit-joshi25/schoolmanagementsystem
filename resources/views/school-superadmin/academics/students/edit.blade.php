...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="bg-white shadow-lg rounded-2xl p-6 md:p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Student: {{ $student->full_name }}</h1>

        <form action="{{ route('school-superadmin.students.update', $student) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8" x-data="studentAdmissionForm()">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="text" name="admission_no" id="admission_no" value="{{ old('admission_no', $student->admission_no) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('admission_no') border-red-500 @enderror">
                    </div>
                    <div>
                        <label for="roll_number" class="block text-sm font-medium text-gray-700">Roll Number</label>
                        <input type="text" name="roll_number" id="roll_number" value="{{ old('roll_number', $student->roll_number) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="text" name="first_name" id="first_name" value="{{ old('first_name', $student->first_name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="last_name" class="block text-sm font-medium text-gray-700">Last Name</label>
                        <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $student->last_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                     ...
                     <div>
                        ...
                        <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $student->date_of_birth) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="category" class="block text-sm font-medium text-gray-700">Category</label>
                        <select name="category" id="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            @foreach($categories as $category)
                            <option value="{{ $category }}" {{ old('category', $student->category) == $category ? 'selected' : '' }}>{{ $category }}</option>
                            @endforeach
                        </select>
                    </div>
                     <div>
                        <label for="religion" class="block text-sm font-medium text-gray-700">Religion</label>
                        <input type="text" name="religion" id="religion" value="{{ old('religion', $student->religion) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                     <div>
                        <label for="caste" class="block text-sm font-medium text-gray-700">Caste</label>
                        <input type="text" name="caste" id="caste" value="{{ old('caste', $student->caste) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                     <div>
                        <label for="mobile_number" class="block text-sm font-medium text-gray-700">Mobile Number</label>
                        <input type="text" name="mobile_number" id="mobile_number" value="{{ old('mobile_number', $student->mobile_number) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="admission_date" class="block text-sm font-medium text-gray-700">Admission Date *</label>
                        <input type="date" name="admission_date" id="admission_date" value="{{ old('admission_date', $student->admission_date) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="house" id="house" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            ...
                            <option value="{{ $house }}" {{ old('house', $student->house) == $house ? 'selected' : '' }}>{{ $house }}</option>
                            @endforeach
                        </select>
                    </div>
                     <div>
                        <label for="height" class="block text-sm font-medium text-gray-700">Height</label>
                        <input type="text" name="height" id="height" value="{{ old('height', $student->height) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="weight" class="block text-sm font-medium text-gray-700">Weight</label>
                        <input type="text" name="weight" id="weight" value="{{ old('weight', $student->weight) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="measurement_date" class="block text-sm font-medium text-gray-700">Measurement Date</label>
                        <input type="date" name="measurement_date" id="measurement_date" value="{{ old('measurement_date', $student->measurement_date) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div class="md:col-span-2">
                        <label for="medical_history" class="block text-sm font-medium text-gray-700">Medical History</label>
                        <textarea name="medical_history" id="medical_history" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('medical_history', $student->medical_history) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        @if($student->student_photo_path)
                            <p class="text-xs text-gray-500 mt-1">Current: <a href="{{ asset('storage/' . $student->student_photo_path) }}" target="_blank" class="text-blue-600">View Photo</a></p>
                        @endif
                    </div>
                </div>
                 <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t">
                     <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $student->email) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('email') border-red-500 @enderror">
                    </div>
                    <div class="md:col-span-2">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('password') border-red-500 @enderror">
                    </div>
                    <div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="guardian_name" id="guardian_name" value="{{ old('guardian_name', $student->guardian_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="guardian_relation" class="block text-sm font-medium text-gray-700">Guardian Relation</label>
                        <input type="text" name="guardian_relation" id="guardian_relation" value="{{ old('guardian_relation', $student->guardian_relation) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="guardian_phone" class="block text-sm font-medium text-gray-700">Guardian Phone</label>
                        <input type="text" name="guardian_phone" id="guardian_phone" value="{{ old('guardian_phone', $student->guardian_phone) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label for="guardian_email" class="block text-sm font-medium text-gray-700">Guardian Email</label>
                        <input type="email" name="guardian_email" id="guardian_email" value="{{ old('guardian_email', $student->guardian_email) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                </div>
            </div>
...
<script>
    function studentAdmissionForm() {
        return {
            ...
            selectedBranchId: '{{ old('branch_id', $student->branch_id) }}',
            selectedClassId: '{{ old('academic_class_id', $student->academic_class_id) }}',
            selectedSectionId: '{{ old('section_id', $student->section_id) }}',
            classes: [],
            sections: [],

...
<div class="bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Certificate Template</h1>
    
    <form action="{{ route('school-superadmin.certificates.update', $certificate) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700">Template Name</label>
                <input type="text" name="name" id="name" value="{{ old('name', $certificate->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('name') border-red-500 @enderror" placeholder="e.g., Transfer Certificate, Bonafide Certificate">
                @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="body" class="block text-sm font-medium text-gray-700">Certificate Body</label>
                <textarea name="body" id="body" rows="15" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm font-mono @error('body') border-red-500 @enderror" placeholder="Write your certificate content here. Use placeholders like [student_name], [class_name], [roll_number], etc.">{{ old('body', $certificate->body) }}</textarea>
